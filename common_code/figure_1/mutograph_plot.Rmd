...
plot.mutograph <- function(input, name, file.type) {
  name2 <- sub("::", "_", name)
  
  if (file.type == "pdf") {
        pdf(paste(name2, "mutograph.pdf", sep = "."),
      width = 8.2677,
      height = 11.6929
  )
...
par(mar = c(0.1, 2, 0.3, 0), oma = c(35, 2, 2, 0), xpd = NA)
...
output.list <- lapply("Lung-AdenoCA::SP52667", process.input)
...
plot.mutograph(output.list[[1]], "Lung-AdenoCA::SP52667", "pdf")
plot.mutograph(output.list[[1]], "Lung-AdenoCA::SP52667", "png")

```

Show the mutograph plot:
```{r, fig.cap='For Figure 1 in manuscript, We manually added the lines between the stacked bars on the left and the corresponding signatures.', fig.topcaption=TRUE, out.width="100%", out.height="50%"}
knitr::include_graphics("Lung-AdenoCA_SP52667.mutograph.png")
```
