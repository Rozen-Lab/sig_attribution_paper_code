...
#
# table_s<X>_summary_stats_<mutation_type>. XXXXX
...
# and, for SBS only
#
# table_s3_fitms_rank_by_rare_threshold_SBS.xlsx
...
compute_and_write_stats <- 
  ...
  # Can factor out the summarize call  
  assessment_each_sample %>%
    group_by(Tool) %>%
    summarize(
...
all_measures <- function(xx, # A data.frame containing expsures. 
  ...
  one.row <- function(ii) {
    ...
    # if (sid == "Breast-AdenoCA::S.30" && tool == "siglasso") {
    #  browser()
    # }
