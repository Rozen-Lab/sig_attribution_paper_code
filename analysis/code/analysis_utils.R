...
# Compute all the statistics comparing the exposures inferred by each tool
# to the ground truth exposures.  Both the inferred exposures and the
# ground truth exposures are in xx.
new_all_stats <- function(xx, mc_cores) {
  mc_cores <- mSigAct:::Adj.mc.cores(mc_cores)
  ...
  one.row <- function(ii) {
    ...
    if (tool == "Ground-Truth") {
      ...
      gt <- gt[, colSums(gt) > 0, drop = FALSE]
      me <- me[, colSums(me) > 0, drop = FALSE]

      tmp <- dplyr::bind_rows(gt, me)
      tmp[is.na(tmp)] <- 0
      ...
      return(list(
        ...
        MD = md, SMD = scaled.manhattan,
        sens = sens, prec = prec, F1 = F1,
        Combined = 1 - scaled.manhattan + F1
      ))
    }
  }
  zz <- parallel::mclapply(1:nrow(xx), one.row, mc.cores = mc_cores)
...
report_error_msg <- function(retval, cancer_type) {
...
execute_pasa <-
  function(spectra, sig, output_dir, num_parallel_samples,
    ...
    retval <- mSigAct::PresenceAssignSigActivity(
      spectra                   = spectra,
      sigs                      = sig,
    ...
    report_error_msg(retval = retval, cancer_type = cancer_type)
    return(retval)
  }

run_pasa_msi <-
  function(spectra, msi_samples, sig, output_dir,
           num_parallel_samples, mc_cores_per_sample, seed_in_use,
           cancer_type) {
