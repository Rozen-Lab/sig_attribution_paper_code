...
cpu_versus_num_sigs = function(mutation_type) {
  ...
  if (mutation_type == "SBS") {
    timings = data.table::fread("output_for_paper/table_s16_cpu_by_cancer_type_SBS.csv")
  } else if (mutation_type == "DBS") {
    timings = data.table::fread("output_for_paper/table_s17_cpu_by_cancer_type_DBS.csv")
  } else if (mutation_type == "ID") {
    timings = data.table::fread("output_for_paper/table_s18_cpu_by_cancer_type_ID.csv")
  }
  ...
  sup_fig_nums = c(SBS = 7, DBS = 8, ID = 9)
...
correlations %>%
  ...
  insert_row(
    paste("Table S19: CPU seconds as a function of",
  ...
  set_valign("middle") %>%
  quick_xlsx(file = "output_for_paper/table_s19_cpu_versus_number_of_sigs.xlsx")
